...
if [[ "${INSTALL_CUDA}" == "true" ]]; then
  DOCKER_IMAGE="${DOCKER_IMAGE}-cuda"
else
  DOCKER_IMAGE="${DOCKER_IMAGE}-nocuda"
fi
if [[ "${INSTALL_OPENVINO}" == "true" ]]; then
  DOCKER_IMAGE="${DOCKER_IMAGE}-openvino"
else
  DOCKER_IMAGE="${DOCKER_IMAGE}-noopenvino"
fi
...
readonly BUILD_BASE_ARGS=(
  ...
  "--build-arg" "install_openvino=${INSTALL_OPENVINO}"
  )
